...
- **Batch_uuid provenance (primary delete cascade):** `add_with_batch_provenance()` mints a `batch_uuid` per `add_memory` call, stores it in every vector payload via `metadata`, and Cypher-tags all graph nodes/edges from that call (Pass A by triple identity + timestamp window + `user_id`; Pass B backfills endpoint nodes because mem0ai's `_add_entities` only sets `created` on NEW nodes, not on endpoints of edges between existing nodes). Pass A deliberately does NOT filter by `run_id`/`agent_id`: mem0ai's `_add_entities` MERGE keys nodes on (name, user_id) only, so an add under `run_id=A` often re-uses pre-existing nodes whose `run_id` is from a DIFFERENT prior add. Filtering Pass A by `s.run_id` would silently miss every relation between cross-scope MERGE'd nodes and leave the batch un-tagged. The process-wide `_graph_lock` keeps the timestamp window single-writer, so identity + window + user_id is sufficient. On delete, `delete_memory_with_batch()` runs the inner `mem.delete()` with `enable_graph=False` to skip mem0ai's re-extraction cascade, counts remaining vector anchors carrying that `batch_uuid` in scope, and calls `hard_delete_batch()` only when anchors hit zero — so multi-memory batches preserve their graph until the last anchor is deleted. Orphan-from-birth (graph relations created but vector path produced no anchors) is reclaimed at add time. Replaces re-extraction-based cleanup, which was non-deterministic on abstract / code-heavy text. Surface contracts pinned in `tests/contract/test_provenance_invariants.py`.
- **Bulk delete batch sweep:** `safe_bulk_delete()` snapshots distinct `batch_uuid`s from the listed payloads BEFORE deleting, then sweeps `hard_delete_batch()` on each after the per-memory deletes complete. Necessary because cross-scope MERGE'd graph nodes (run_id mismatch between the bulk filter and the actual node properties) can't be reached by mem0ai's `graph.delete_all(filters)` — only by tag. The filter-based `graph.delete_all` still runs as a fallback for legacy / un-tagged elements.
...
- **Add-time tagging missed**: Pass A returned 0 — most often a sanitizer/normalization mismatch between mem0ai's `added_entities` shape and stored Neo4j `type(r)`. Inspect `tests/contract/test_provenance_invariants.py` for the surface contract. (Historical note: prior to 2026-05-01 Pass A also filtered by `run_id`/`agent_id` on endpoint nodes, which silently failed when mem0ai's MERGE re-used pre-existing nodes from a different scope. That filter has been dropped — see `_PASS_A_TAG_RELATIONS_CYPHER` rationale.)
